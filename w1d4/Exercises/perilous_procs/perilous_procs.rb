...
def first_index(array, &prc)
    array.each_with_index {|ele,i| return i if prc.call(ele)}
    nil
end
...
def multi_map(array, n=1, &prc)
    ...
    n.times do
        new_array.map! {|ele| prc.call(ele)}
    end
    new_array
end
...
def proc_suffix(sentence, hash)
    ...
    words.map! do |word|
        hash.each { |prc,suffix| word += suffix if prc.call(word)}
        word
    end
    words.join(" ")
end
...
def procipher(sentence, hash)
    ...
    words.map! do |word|
        hash.each do |prc_1, prc_2|
            word = prc_2.call(word) if prc_1.call(word)
        end
        word
    end
